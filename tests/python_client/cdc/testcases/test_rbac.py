        ...
        def check_grant():
            ...
            try:
                ...
                for privilege_info in role_info['privileges']:
                    ...
                    print(f"DEBUG: privilege_info.get('object_type') in check_grant: {privilege_info.get('object_type')}")
                    print(f"DEBUG: privilege_info.get('privilege') in check_grant: {privilege_info.get('privilege')}")
                    print(f"DEBUG: privilege_info.get('object_name') in check_grant: {privilege_info.get('object_name')}")
                    if (privilege_info.get('object_type') == "Collection" and
                        privilege_info.get('privilege') == "Search" and
                        privilege_info.get('object_name') == "*"):
                        return True
                return False
            except Exception as e:
    ...
    def test_grant_privilege_v2(self, upstream_client, downstream_client, sync_timeout):
        ...
        def check_grant():
            ...
            try:
                role_info = downstream_client.describe_role(role_name)
                for privilege_info in role_info['privileges']:
                    if (privilege_info.get('privilege') == "Query" and
                        privilege_info.get('object_name') == "*"):
                        return True
                return False
            except Exception as e:
