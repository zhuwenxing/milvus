...
class BulkInsertChecker(Checker):
    ...
    def __init__(self, collection_name=None, files=[], use_one_collection=False, dim=ct.default_dim,
                 ...
                 downstream_minio_endpoint=None, downstream_bucket_name=None
                 ):
        if collection_name is None:
        ...
        self.downstream_minio_endpoint = downstream_minio_endpoint
        self.downstream_bucket_name = downstream_bucket_name
    ...
    def prepare(self, data_size=1000):
        with RemoteBulkWriter(
            ...
            for _ in range(data_size):
                ...
                row[self.int64_field_name] = int(time.time() * self.scale)
        ...
        if self.downstream_minio_endpoint is not None and self.downstream_bucket_name is not None:
            ...
            syncer = MinioSyncer(
                ...
                dst_endpoint=self.downstream_minio_endpoint,
                dst_access_key="minioadmin",
                dst_secret_key="minioadmin",
                secure=False
            )
            # Sync only the specific files generated by RemoteBulkWriter
            all_files = [file for batch in self.batch_files for file in batch]  # Flatten the list of files
            log.info(f"all files: {all_files}")
            success, errors, skipped = syncer.sync_files(
                src_bucket=self.bucket_name,
                dst_bucket=self.downstream_bucket_name,
                files=all_files
            )
            if success != len(all_files):
                log.error(f"Sync files failed, success: {success}, errors: {errors}, skipped: {skipped}")
                raise Exception(f"Sync files failed, success: {success}, errors: {errors}, skipped: {skipped}")
            log.info(f"Sync files success, success: {success}, errors: {errors}, skipped: {skipped}")
