...
class TestCollectionProperties(TestBase):
    ...
    def test_alter_index_properties(self):
        ...
        method: create collection, alter index properties
        expected: alter index properties success
        """
        name = gen_collection_name()
        client = self.collection_client
        ...
        self.index_client.index_list(name)
        # create index
        payload = {
            "collectionName": name,
            "indexParams": [
        ...
        rsp = self.index_client.index_describe(name, "book_intro_vector")
        # alter index properties
        properties = {"mmap.enabled": True}
        rsp = client.alter_index_properties(name, "book_intro_vector", properties)
        assert rsp['code'] == 0
        rsp = self.index_client.index_describe(name, "book_intro_vector")
    ...
    def test_drop_index_properties(self):
        """
        target: test drop index properties
        method: create collection, alter index properties, drop index properties
        expected: drop index properties success
        """
        name = gen_collection_name()
        dim = 128
        client = self.collection_client
        ...
        # alter index properties
        properties = {"mmap.enabled": True}
        rsp = client.alter_index_properties(name, "my_vector", properties)
        assert rsp['code'] == 0

        # drop index properties
        delete_keys = ["mmap.enabled"]
        rsp = client.drop_index_properties(name, "my_vector", delete_keys)
        assert rsp['code'] == 0
